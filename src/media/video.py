...
def iter_frames_pyav(
    ...
    *,
    ...
    A fixed filter graph gets confused. We detect changes and rebuild the graph on the next frame.
    """
    config = Config()
    TW, TH = size
    ...
    try:
        container, vstream = open_with_hwaccel(real_src_url, hw_prefer, options=http_opts)
        if vstream is None:
            raise RuntimeError("no video stream")
        ...
        if isinstance(e, FileNotFoundError) or "no such file" in msg or "not found" in msg:
            raise FileNotFoundError(f"cannot open source: {src_url}") from e
