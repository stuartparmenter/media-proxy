...
def get_ffmpeg_exe_path() -> str | None:
...
def _build_hw_backend_cache():
    ...
    try:
        out = subprocess.check_output([exe, "-hide_banner", "-hwaccels"], text=True, stderr=subprocess.STDOUT)  # noqa: S603  # ffmpeg path from config, not user input
        return {line.strip().lower() for line in out.splitlines() if line.strip()}
    except Exception:
        return set()


def pick_hw_backend(prefer: str | None = None) -> str | None:
