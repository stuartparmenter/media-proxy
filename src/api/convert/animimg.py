...
def create_esphome_yaml_config(frame_files: List[Dict[str, Any]]) -> str:
    ...
    for frame_info in frame_files:
        image_configs.append({
            ...
            'id': f"frame_{frame_info['file'].replace('.png', '').replace('frame_', '')}"
        })
            ...
            'widgets': [
                {
                    'animimg': {
                        'id': 'my_animation',
                        'src': [frame_info['file'] for frame_info in frame_files],
                        'duration': sum(frame_info['delay_ms'] for frame_info in frame_files),
                        'repeat': {
                            'count': 'LV_ANIM_REPEAT_INFINITE'
                        }
                    }
                }
            ]
...
#
# Usage:
# 1. Copy all PNG files to your ESPHome config directory
# 2. Add the image and lvgl sections to your ESPHome YAML
# 3. Customize the animimg widget properties as needed
#
# Frames: {len(frame_files)}
# Total duration: {sum(frame_info['delay_ms'] for frame_info in frame_files)}ms
